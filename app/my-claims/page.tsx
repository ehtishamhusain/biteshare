      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        >
          ...
          <p className="text-slate-600 text-sm">
            Show your 4-Digit Security PIN at the store counter to collect your fresh surplus food.
          </p>
        </motion.div>
        ...
        {loading ? (
          ...
          >
            {claims.map((claim) => {
              ...
              const qtyReserved = claim.claimed_quantity || 1;
              const pricePaid =
                claim.total_price ??
                (bundle?.price_per_item
                  ? qtyReserved * bundle.price_per_item
                  : bundle?.price || 0);
                ...
                >
                  <div className="space-y-3 flex-grow">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="px-2.5 py-0.5 rounded-lg bg-emerald-50 text-emerald-800 border border-emerald-200 text-xs font-extrabold flex items-center gap-1">
                        <ShoppingBag className="w-3 h-3 text-emerald-600" />
                        <span>Reserved: {qtyReserved} item(s)</span>
                      </span>

                      <span className="font-black text-sm text-slate-900 bg-slate-100 px-2.5 py-0.5 rounded-lg border border-slate-200">
                        {pricePaid === 0 ? '🎁 FREE' : `₹${pricePaid} Total`}
                      </span>
                    </div>
