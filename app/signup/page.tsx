...
import { Utensils, Heart, ArrowRight, AlertCircle, Eye, EyeOff, Check, X } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (data.user) {
      // 2. Pre-create basic profile row in database to prevent foreign key errors
      ...
      // 3. STEP 1 COMPLETE: Redirect user to Profile Setup first!
      router.push('/profile');
    } else {
      setLoading(false);
    }
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-6 shadow-sm border border-slate-200 rounded-2xl sm:px-10">
          ...
          <form className="space-y-6" onSubmit={handleSignUp}>
            ...
            >
              <span>{loading ? 'Creating Account...' : 'Continue to Setup Profile'}</span>
